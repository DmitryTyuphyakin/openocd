...
static int __poll_busy(struct flash_bank *bank, size_t wait_s)
{
    ...
    while (alive) {
        cmd[0] = W25Q256FV_RDSR1_OPCODE;
        retval = info->xfer(bank, cmd, 1, NULL, 0, &status, 1);
        if (retval != ERROR_OK) return retval;

        alive = status & W25Q256FV_SR1_BUSY_FLAG;
...
{
    struct stmspi_flash_bank *info = bank->driver_priv;
    uint8_t cmd = (enable) ? W25Q256FV_WREN_OPCODE
                           : W25Q256FV_WRDI_OPCODE;
...
#else
    struct stmspi_flash_bank *info = bank->driver_priv;
    uint8_t cmd = W25Q256FV_RDSR1_OPCODE;
    uint8_t status = 0;
...
static int __erase_sector(struct flash_bank *bank, size_t idx)
{
    ...
    int retval = ERROR_OK;
    // uint8_t opcode = W25Q256FV_P4E_OPCODE;
    uint8_t cmd[] = {
        W25Q256FV_P4E_OPCODE,
...
int w25q256fv_configure(struct flash_bank *bank)
{
    ...
    // Check flash exists
    cmd[0] = W25Q256FV_RDID_OPCODE;
    retval = info->xfer(bank, cmd, 1, NULL, 0, &jedec_info, sizeof(jedec_info));
    if (retval != ERROR_OK) return retval;

    if ( (jedec_info.manufacturer != W25Q256FV_MANUFACTURER_ID) ||\
         (jedec_info.device != W25Q256FV_DEVICE_ID)) {
        return ERROR_FAIL;
    }

    // Goto 4-byte instruction
    cmd[0] = W25Q256FW_4BAME_OPCODE;
    ...
    bank->size = W25Q256FV_SIZE;
    bank->num_sectors = W25Q64FL_SECTOR_NUMBER;
    bank->sectors = calloc(bank->num_sectors, sizeof(struct flash_sector));
    if (bank->sectors == NULL) return ERROR_FAIL;

    while (idx<bank->num_sectors) {
        bank->sectors[idx].offset       = offset;
        bank->sectors[idx].size         = W25Q64FL_SECTOR_SIZE;
        bank->sectors[idx].page_size    = W25Q64FL_PAGE_SIZE;
        bank->sectors[idx].is_erased    = false;
        bank->sectors[idx].is_protected = 0;

        offset += W25Q64FL_SECTOR_SIZE;
        idx++;
    }
...
int w25q256fv_erase(struct flash_bank *bank, unsigned int first, unsigned int last)
{
    int retval = ERROR_OK;
...
int w25q256fv_read(struct flash_bank *bank, uint8_t *out,
                   uint32_t offset, uint32_t size)
{
    struct stmspi_flash_bank *info = bank->driver_priv;
    int retval = ERROR_OK;
...
int w25q256fv_write(struct flash_bank *bank, const uint8_t *buffer,
